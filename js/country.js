(() => {
	...
	// 			messageA_opacity_in: [0,1, {start:0.15, end:0.25}],
	// 			messageB_opacity_in: [0,1, {start:0.39, end:0.45}],
	// 			messageC_opacity_in: [0,1, {start:0.6, end:0.75}],

	// 			messageA_translateY_in: [20, 0, {start:0.15, end:0.25}],
	// 			messageB_translateY_in: [20, 0, {start:0.39, end:0.45}],
	// 			messageC_translateY_in: [20, 0, {start:0.6, end:0.75}],
				
	// 			messageA_opacity_out: [1, 0, {start:0.3, end:0.38}],
	// 			messageB_opacity_out: [1, 0, {start:0.5, end:0.6}],
	// 			messageC_opacity_out: [1, 0, {start:0.8, end:0.9}],

	// 			messageA_translateY_out: [0, -20, {start:0.3, end:0.38}],
	...
	// food 화살표 이벤트
	const left = document.querySelector('.food-left');
	const right = document.querySelector('.food-right');
	...
	left.addEventListener('click', () => {
		...
		for (let i=0; i<cards.length; i++) {
			cards[i].style.transform = `translate3d(${movement}px, 0, 0)`;
		}
	}) 

	right.addEventListener('click', () => {
		if (movement == (-1)*cardSize*(cards.length-4)) return;
		movement -= cardSize;
		for (let i=0; i<cards.length; i++) {
			cards[i].style.transform = `translate3d(${movement}px, 0, 0)`;
		}
